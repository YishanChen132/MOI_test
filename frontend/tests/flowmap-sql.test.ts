...
describe('flowmap source SQL', () => {
  ...
  it('uses road-node-transition sources for the node flowmap preset', () => {
    const query = buildFlowmapSourceQuery(buildAppliedScenario('taipei_road_node_flowmap'));
  ...
  it('builds a road-flow join query for the path-based road renderer', () => {
    const query = buildRoadFlowSourceQuery(buildAppliedScenario('taipei_edge_flowmap'));
