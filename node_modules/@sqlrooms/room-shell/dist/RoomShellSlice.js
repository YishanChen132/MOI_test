...
import { DataSourceTypes, createDefaultBaseRoomConfig, isFileDataSource, isSqlQueryDataSource, isUrlDataSource, } from '@sqlrooms/room-config';
import { createBaseRoomSlice, createCommandSlice, isRoomSliceWithInitialize, registerCommandsForOwner, unregisterCommandsForOwner, useBaseRoomStore, } from '@sqlrooms/room-store';
...
export function createRoomShellSlice(props) {
    const slice = (set, get, store) => {
        const { connector, config: configProps, layout: layoutProps, room: deprecatedRoomProps, fileDataSourceLoader, CustomErrorBoundary = ErrorBoundary, createCommandProps, captureException = (exception) => console.error(exception), ...restProps } = props;
        ...
        const sliceState = {
            ...
            room: {
                ...
                async initialize() {
                    registerCommandsForOwner(store, ROOM_SHELL_COMMAND_OWNER, createRoomShellCommands());
                    ...
                    try {
                        ...
                        const slices = Object.entries(store.getState()).filter(([key, value]) => key !== 'room' &&
                            key !== 'db' &&
                ...
                async addOrUpdateSqlQueryDataSource(tableName, query, oldTableName, abortSignal) {
                    const { schema } = get().db;
                    const { db } = get();
                    ...
                    if (dbConnectors?.runQuery) {
                        ...
                        rowCount = await db.getTableRowCount(relation, schema);
                    }
                    else {
                    ...
                    set((state) => produce(state, (draft) => {
                        ...
                        if (oldTableName) {
                            draft.room.config.dataSources =
                                draft.room.config.dataSources.map((dataSource) => dataSource.tableName === oldTableName
                                    ? newDataSource
                                    : dataSource);
                ...
                removeDataSource: async (tableName) => {
                    const { db } = get();
                    const dataSource = get().room.config.dataSources.find((d) => d.tableName === tableName);
                    if (dataSource) {
                        set((state) => produce(state, (draft) => {
                            draft.room.config.dataSources =
                                draft.room.config.dataSources.filter((d) => d.tableName !== tableName);
                            if (dataSource && isFileDataSource(dataSource)) {
                                draft.room.roomFiles = draft.room.roomFiles.filter((f) => f.pathname !== dataSource.fileName);
                            }
                ...
                removeRoomFile: async (pathname) => {
                    const dataSource = get().room.config.dataSources.find((d) => isFileDataSource(d) && d.fileName === pathname);
                    if (dataSource) {
        ...
        async function downloadRoomFiles(filesToDownload) {
            ...
            const loadedFiles = await Promise.all(filesToDownload.map(async (ds) => {
                ...
                try {
                    ...
                    updateTotalFileDownloadProgress();
                    const { db } = get();
                    await db.connector.loadFile(downloadedFile instanceof File
                        ? downloadedFile
                        : new File([new Uint8Array(downloadedFile)], fileName), ds.tableName, ds.loadOptions);
            ...
            for (const ds of queries) {
                try {
                    ...
                    if (dbConnectors?.runQuery) {
                        ...
                        rowCount = await get().db.getTableRowCount(relation);
                    }
                    else {
                        const created = await get().db.createTableFromQuery(tableName, sqlQuery);
                        rowCount = created.rowCount;
                    }
                    if (rowCount !== undefined) {
...
function createRoomShellCommands() {
    return [
        {
            id: 'room.set-title',
            name: 'Set room title',
        ...
        },
        {
            ...
            validateInput: (input, { getState }) => {
                ...
                const dataSource = getState().room.config.dataSources.find((source) => source.tableName === tableName);
                if (!dataSource) {
                    throw new Error(`Data source for table "${tableName}" was not found.`);
                }
            },
        ...
        },
        {
            ...
            inputDescription: 'Provide url and tableName, with optional httpMethod and headers.',
            metadata: {
                readOnly: false,
                idempotent: false,
                riskLevel: 'medium',
            ...
            execute: async ({ getState }, input) => {
                const { url, tableName, httpMethod, headers } = input;
...
//# sourceMappingURL=RoomShellSlice.js.map