...
export function createHttpDbBridge(options) {
    ...
    const request = async (path, init) => {
        ...
        if (!res.ok) {
            ...
            throw new Error(`Bridge request failed (${res.status}) for ${path}${details ? `: ${details}` : ''}`);
        }
        return (await res.json());
    };
    const cancelQuery = async (queryId) => {
        const result = await request('/api/db/cancel-query', {
            method: 'POST',
    ...
    return {
        ...
        testConnection: async (connectionId) => {
            const result = await request('/api/db/test-connection', {
                method: 'POST',
        ...
        fetchArrow: async ({ connectionId, sql, signal }) => {
            ...
            if (!res.ok) {
                const details = await getErrorDetails(res);
                throw new Error(`Bridge fetchArrow failed (${res.status})${details ? `: ${details}` : ''}`);
            }
        ...
        fetchArrowStream: ({ connectionId, sql, signal, queryId, chunkRows }) => {
            ...
            return (async function* () {
                ...
                if (!res.ok) {
                    const details = await getErrorDetails(res);
                    throw new Error(`Bridge fetchArrowStream failed (${res.status})${details ? `: ${details}` : ''}`);
                }
                if (!res.body) {
